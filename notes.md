...
- client always initiates request and server responds
- however applications for notificaitons, distributed task processing, peer to peer communication, or asynchronous events need communication initaed by two or moer connected devices
- used to hack to work around limitation of client/server model. 
  - have client frequntly pinging server to see if it had anything new
  - keep client-initated connections open for a long time as client waited for an event to happen on server
  - not elegant/efficient
- in 2011 communication protocol WebSocket was created
  - fully duplexed - after initial connection is made from client using vanilla HTTP, and then upgraded by server to a WebSocket connection, relationship changes to a peer-to-peer connection where either party can efficiently send data at any time

  ![alt text](https://raw.githubusercontent.com/webprogramming260/.github/main/profile/webServices/webSocket/webServicesWebSocketUpgrade.jpg)

  - WebSockes are still only between two parties. If you want to facilitate a conversation between groups of users the server acts as intermediary.
     - EAch peer first connects to server, then server forawrds messages amongst peers
### Creating a WebSocket Conversation
- JS running on browser can intiate WebSocket connection with browser's WebSocket API. 
  - First create WebSocket object by specifying what part you want to communicate on
  - then can send messages with `send` function and register callback using `onmessage` funciton to receive messages
  ```
  ...
  ```
- server uses `ws` package to create WebSocketServer that's listeneing on the same port as the browser. By specifiying a port when you create a WSS you are telling server to listen for HTTP connects on that port and to automatically upgrade them to a WebSocket connection is the request has `connection: Upgrade` header. 
- when connection is detected it calls server's `on connection` call back. Server can then send messages with `send` function and register callback using `on message` function to receive messages
...
```
## Debugging WebSocket
- you can debug  both sides of the WebSocket communication with VScode to debug the server, and Chrome to debug the client.
### Debugging the Server
1. create a directory named `testWebSocket` and change to that directory
2. run `npm init -y`
3. run `npm install ws`
4. open VSCode and create file `main.js` paste the following code
```
const { WebSocketServer } = require('ws');
...
```
5. set breakpoints on `ws.send` lines so you can inspect code executing
6. start debugging `f5`
### Debugging the Client
1. open the Chrome debugger by pressing `F12`
2. paste this code into the debugger console window and press enter to execute. Executing this code will immediately hit the server breakpoint. Take a look at what's going on and remove breakpoint
```
const socket = new WebSocket('ws://localhost:9900');
...
```
3. select `network` tab and select HTTP message that was generated by the execution of the above client code
4. with the HTTP message selected, click `messages` tab to view the WebSocket messages
5. send a message to the server by executing the following in the debugger console. Hits the second breakpoint, explore then remove 
```
socket.send('I am listening');
```
...
```
- JS for the application provides interaction with the DOM for creating and displaying messages. Manages WebSockets to connect, send, and receive messages
#### DOM Interaction
- don't want to be able to send messages if User doesn't have name. Add event listener on name input and disable chat controls if it's empty
```
...
```
- create function that updates displayed messages by selecting element with `chat-text` ID and appending new message to its HTML. (will allow any chat user to execute code in context of application)
```
...
```
- when user presses enter in message nput we wnat to send the message to the socket. Add a listener for the `Enter` keystroke
```
...
```
- when Enter is pressed, sendMessage function is called. Selects text from `new-msg` element and name from `my-name` element sends it to the WebSocket. message is then cleared and is ready for the next
```
...
- want to support secure and non-secure WebSocket connections. Look at current protocol represensted by `window.location.protocol` variable. If it's non-secure htTP then we set WebSocket protocol to be non-secure WebSocket (`ws`), otherwise use secure (`wss`). 
- can then use to connect WebSocket to same location that we loaded the HTML from by reerencing the `window.locaiton.host` variable. 
- notify user chat is ready to go by listening to `onopen` event and appending text
```
...
```
- when WebSocket receives message from peer it displays it ussing `appendMsg` function
```
...
```
- if WebSocket closes, we display to user and disable controls
```
...
```
### Chat Server
- chat server runs web service, serves up client code, manages WebSocket connections, and forwards messages from peers
#### Web Service
 - established using Expcress. Serve up client HTML, CSS, and JS using `static` middleware
 ```
 const { WebSocketServer } = require('ws');
const express = require('express');
const app = express();
...
```
#### WebSocket SErver
- instead of letting WebSocketServer control both HTTP connection and upgrading websocket, we want to use the HTTP connection provided by Express and handle upgrade to WebSocket ourselves. 
- speciffy `noServer`option when creating WebSocketServer and handling `upgrade` notification that occurs when client requests ugrade of protocol from HTTP to WebSocket
```
...
```
#### Forwarding Messages
 - with WebSocket Server we can use `connection`, `message`, and `close` events to forward messages between peers. 
 - on connection insert object representing the connection into a list of all connection from chat peers
- when messages is received we loop through peer connections and forward to everyone but who initated 
- finally remove connection from peer connection list when it's closed
```
...
- WebSocket connection will eventually close automatically if no data is sent across it
- to prevent: WS protcol suports ability to send `ping` message to see if peer is still there and receive ` pong ` responses to indicate affirmative. 
- use `setInterval` to send ping ever 10 sec to each of our peer connections and clean up any connections that didn't respond to our previous ping
```
setInterval(() => {
  connections.forEach((c) => {
...
```
- in our `connection` handler we listen for `pong` response and mark connection as alive
```
